...
// Source File Name:   BytesEncodingDetect.java

package com.timerchina.spider.service.encoding;
...
// Referenced classes of package com.timerchina.spider:
//            Encoding
...
public class BytesEncodingDetect extends Encoding
{
    ...
    public static void main(String argc[])
    {
        ...
        for(int i = 0; i < argc.length; i++)
        {
            if(argc[i].startsWith("http://"))
            {
                ...
                {
                    System.err.println((new StringBuilder("Bad URL ")).append(e.toString()).toString());
                }
            } else
            {
                if(argc[i].equals("-d"))
    ...
    public int detectEncoding(URL testurl)
    {
        ...
        {
            System.err.println((new StringBuilder("Error loading or using URL ")).append(e.toString()).toString());
            guess = -1;
        }
        return guess;
    }
...
//      scores[BIG5] = big5_probability(rawtext);
        ...
        {
            if(debug)
                System.out.println((new StringBuilder("Encoding ")).append(nicename[index]).append(" score ").append(scores[index]).toString());
            if(scores[index] > maxscore)
            {
            	encoding_guess = index;
            	if(nicename[index].equalsIgnoreCase(STR_GBK)){
            		gbkScores = scores[index];
            	}
        ...
        if(maxscore <= 50)
            encoding_guess = GBK;
        LoggerHandler.getInstance().writeLogger("编码识别最高分："+maxscore+"  GBK得分："+gbkScores, Constant.LOG_LEVEL_INFO);
    ...
    int gb2312_probability(byte rawtext[])
    {
        ...
        freqval = 50F * ((float)gbfreq / (float)totalfreq);
    ...
    int gbk_probability(byte rawtext[])
    {
        ...
        for(int i = 0; i < rawtextlen - 1; i++)
            if(rawtext[i] < 0)
            {
                ...
                if(-127 <= rawtext[i] && rawtext[i] <= -2 && (-128 <= rawtext[i + 1] && rawtext[i + 1] <= -2 || 64 <= rawtext[i + 1] && rawtext[i + 1] <= 126))
                {
                    gbchars++;
                    totalfreq += 500L;
    ...
    int gb18030_probability(byte rawtext[])
    {
        ...
        for(int i = 0; i < rawtextlen - 1; i++)
            if(rawtext[i] < 0)
            {
                ...
                if(-127 <= rawtext[i] && rawtext[i] <= -2 && i + 1 < rawtextlen && (-128 <= rawtext[i + 1] && rawtext[i + 1] <= -2 || 64 <= rawtext[i + 1] && rawtext[i + 1] <= 126))
                {
                    gbchars++;
                    totalfreq += 500L;
                    int row = (rawtext[i] + 256) - 129;
    ...
    int hz_probability(byte rawtext[])
    {
            ...
            if(rawtext[i] == 126)
                if(rawtext[i + 1] == 123)
                {
                    ...
                    for(i += 2; i < rawtextlen - 1; i += 2)
                    {
                        ...
                        if(161 <= rawtext[i] && rawtext[i] <= 247 && 161 <= rawtext[i + 1] && rawtext[i + 1] <= 247)
                        {
                            hzchars += 2;
                            int row = (rawtext[i] + 256) - 161;
                            int column = (rawtext[i + 1] + 256) - 161;
        ...
        if(hzstart > 0)
            ...
            rangeval = 0.0F;
        freqval = 50F * ((float)hzfreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
    ...
    int big5_probability(byte rawtext[])
    {
        ...
        freqval = 50F * ((float)bffreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
    ...
    int euc_tw_probability(byte rawtext[])
    {
        ...
        rangeval = 50F * ((float)cnschars / (float)dbchars);
        freqval = 50F * ((float)cnsfreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
    ...
    int iso_2022_cn_probability(byte rawtext[])
    {
        ...
        for(int i = 0; i < rawtextlen - 1; i++)
            if(rawtext[i] == 27 && i + 3 < rawtextlen)
            {
                ...
                if(i + 3 < rawtextlen && rawtext[i + 1] == 36 && rawtext[i + 2] == 41 && rawtext[i + 3] == 71)
                    for(i += 4; rawtext[i] != 27; i++)
                    {
                        dbchars++;
        ...
        rangeval = 50F * ((float)isochars / (float)dbchars);
        freqval = 50F * ((float)isofreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
    ...
    int euc_kr_probability(byte rawtext[])
    {
        ...
        freqval = 50F * ((float)krfreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
    ...
    int euc_jp_probability(byte rawtext[])
    {
        ...
        freqval = 50F * ((float)jpfreq / (float)totalfreq);
        return (int)(rangeval + freqval);
    }
...
/*
	...
	Decompiled from: D:\Myeclipse8.5 WorkSpace\Spider2014_V3.0\lib\spider2012.jar
	Total time: 1003 ms
